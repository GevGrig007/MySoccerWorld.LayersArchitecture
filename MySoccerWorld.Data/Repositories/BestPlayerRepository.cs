...
namespace MySoccerWorld.Data.Repositories
{
    public class BestPlayerRepository : IBestPlayerRepository
    {
        ...
        public Task<List<BestPlayer>> GetByTournamentAsync(int id) =>
               ...
               .Include(b => b.PlayerTeam).ThenInclude(p => p.Team).Where(p => p.TournamentId == id).ToListAsync();
        ...
        public IEnumerable<Player> GetPlayerForAwards(int id) =>
               _context.Players.Where(p => p.PlayerTeams.Any(p => p.BestPlayers.Any(b => b.TournamentId == id))).Include(p => p.PlayerTeams).Include(p => p.Country);
        public async Task UpdateAsync(BestPlayer bestPlayer)
        {
            if (bestPlayer.Id == 0)
